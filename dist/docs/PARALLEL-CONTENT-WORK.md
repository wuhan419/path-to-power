...
| `events/134—137` | 已占用（登记补记） | 134 = 2025 收官补钉（#32④）、136/137 = 两条后话链（911 / 次贷，见 `CONTENT-SCHEMA.md` §4.21）；135 空位 |
| `events/138` | **主题包（已启用）** | 138 = 投机包 `138-speculation.js`（#28③【庄家生意】，id 前缀 `sp{YY}_`，六张全走 `pace:"exempt"` 豁免通道，见 `CONTENT-SCHEMA.md` §4.21） |
| `events/139, 141—149` | 预留（空位） | 主题包/补薄备用段，未经本表登记不得占用 |
...
逐年 Σ 最低 4，**无偏薄年份**。每轮补薄后重跑一遍逐年表复核，别拿均值充数：

```bash
node dev/tools/density-scan.js          # 默认量 dev/；有偏薄年份时退出码 1
node dev/tools/density-scan.js --root=dist   # 量发布件
```
