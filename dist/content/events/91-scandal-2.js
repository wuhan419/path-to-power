...
POTUS.define("event", [
   ...
   * ======================================================================== */
  {
    ...
    body: "竞选部的实习生发现有人在校友会办公室查过你的旧档案。\n" +
      "不是记者 —— 记者会直接打电话来。这是一家「研究公司」：对手雇来翻旧档的挖坟队，按页付钱，\n" +
      "从年鉴翻到毕业名册。你的大学时代确实有几页会被标题化 —— 不是犯罪，但会被放大。\n" +
      "他们翻到了也不会现在用 —— 要等你最经不起的那一周。他们手里是传闻还是复印件，只有他们自己知道。",
    choices: [
      {
   ...
   * ======================================================================== */
  {
    ...
    body: "那天晚上是在一个朋友家的书房里，在座的你全认识。\n" +
      "你说了一段话 —— 关于选民、关于金钱、关于你真正怎么看那件事。说得很痛快。\n" +
      "现在有人把那段痛快装在一件东西里，寄给了你：没有署名，只有一句话：「你觉得它值多少？」\n" +
      "那句话单看是断章 —— 剪掉语境的一小段 —— 连前后文一起听完才致命。书房里除你只有三个人，录你的就在其中。\n" +
      "他要的是钱、是把柄，还是要你长期听话？在付第一分钱之前，你不会知道。",
    choices: [
      {
   ...
   * ======================================================================== */
  {
    ...
    grade: "mid", valence: "bane", dyn: true, category: "scandal",
    title: "凌晨两点的保释电话",
    body: "你弟弟在郊区被拦下来了。酒驾，车里还坐着一个你竞选金主的儿子 —— 他父亲正在给你写支票。\n" +
      "按程序他会在天亮前被保释 —— 按程序这件事也会出现在明天的地方版上。\n" +
      "他在电话里只说了一句话：「哥，别告诉妈。」\n" +
      "你握着电话，想起他为你做过的每一次站台、每一块标语牌。你知道这不是第一次。\n" +
      "救他、切割他（公开划清界限），还是陪他挨这一刀 —— 他会感激你，还是认定你欠他？",
    choices: [
      {
        ...
        outcomes: {
          ...
          meh: s2Out("名字按住了，程序没按住：他被吊销驾照九十天，而你多了一个知道你全部底细的中间人。",
            { contact: { fixer: 4 }, flags: ["sca2_family_hidden"] }),
   ...
   * ======================================================================== */
  {
    ...
    body: "信封里是一张复印件：十年前的支票存根、一个你几乎忘了的账号、\n" +
      "还有用红笔圈出来的一个数字 —— 你当年拿过的那笔灰色启动资金。\n" +
      "寄信人不是记者，不是检察官，是一个你此刻才想起来还活着的名字 —— 当年过手的中间人。放钱的人早死了，账簿留在他手上。\n" +
      "他只附了一行字：「当年说好的，不是这个价。」他要两样：一笔保管费，和你逢人继续说那套旧口径 —— 当年统一好的说法，如今也是破绽。\n" +
      "这一页复印件在外面流通的还有几份，你猜不到。",
    choices: [
      {
      ...
      },
      {
        ...
        outcomes: {
          ...
          meh: s2Out("你交了，检方却更想要大鱼：他们要你继续戴着窃听器去见他。你从此活在两面派的每一天里。",
            { fac: { agency: 5 }, flags: ["compromised"] }),
   ...
   * ======================================================================== */
  {
    ...
    body: "那件事压不住了 —— 它不是今天发生的，是今天到期的。主编室里已经排好了版，律师的电话在桌上震动，\n" +
      "竞选经理站在窗前背对着你，问了一个你现在必须回答的问题：「我们到底走哪条路？」\n" +
      "否认的窗口只开到证据链闭合那一刻；切割的账很诚实 —— 扔出去的人要够大，否则只是延时；\n" +
      "反诉能翻盘，也最可能加倍 —— 你的案卷会先进同一个法官的抽屉。\n" +
      "有人说主编手里只有半份材料，在赌你自己供出另一半。缺的那一环，此刻在谁手里？",
    choices: [
      {
   ...
   * ======================================================================== */
  {
    ...
    body: "当初那件事有了结局 —— 或者说，有了第一版结局。第一版从来不是最终版。\n" +
      "你以为账已经结了。今天寄到的是利息 —— 被按下的东西，隔几年就要续一次费。\n" +
      "寄账单的人各不相同，但账单的抬头都是同一行：你在那四十八小时里选的那条路。\n" +
      "你要决定的不是那件事，是往后按哪个版本说。那一版里会不会有一句你没料到的话？",
    choices: [
      {
