    ...
    choices: [
      {
        ...
        cost: { fun: 800, ap: 1 }, stake: { ap: true },
        outcomes: {
          ...
          meh: aOut("复印到一半机器卡纸。你抽出来的那页缺了右下角，正好是日期。",
            { lev: 1, hp: -3, flags: ["archive_taken"] }),
          fail: aOut("你只来得及复印前半份。后半份里有什么，你以后会一直想知道。",
            { lev: 1, hp: -4, flags: ["archive_taken"] }),
          critfail: aOut("归档员提前回来了。你说你在找洗手间，他信了。但复印机的计数器上多了一个数字。",
            { lev: 1, hp: -5, fac: { agency: -6 }, flags: ["archive_taken", "archive_seen"] })
        }
      },
      {
      ...
      },
      {
        ...
        outcomes: {
          ...
          critfail: aOut("你在里面站得太久。有人报告说地下室的灯亮过整个午休时段。这件小事以后会在某个不合适的时候被想起来。",
            { rep: -2, fac: { agency: -5 }, flags: ["archive_seen"] })
        }
      }
    ]
  },
    ...
    choices: [
      {
        ...
        cost: { fun: 40000, ap: 2 }, stake: { fun: true, ap: true },
        outcomes: {
          crit: aOut("三天后你手里有了一份车牌、一个办公地址和一个不该出现的人名。你把这三样东西装进信封，寄到了她的办公地址。她没有再出现。",
            { lev: 1, rep: 3, contact: { fed: 8 }, fac: { agency: 8 }, flags: ["archive_hunter"] }),
          ...
          fail: aOut("你派去的人被她认出来了。她从那天起换了车，也换了一个更有耐心的做法。",
            { rep: -3, hp: -4, fac: { agency: -8 }, flags: ["archive_seen"] }),
          critfail: aOut("你查她的动作，比你翻档案的动作更清楚地说明了一件事：那份东西是真的，而且你很在乎。",
            { rep: -5, hp: -5, fac: { agency: -12 }, flags: ["archive_seen", "investigation_open"] })
        }
      },
      {
        ...
        outcomes: {
          crit: aOut("他没有立刻写。他把复印件的每一页都做了编号，然后当着你的面锁进了保险箱。「等他们先动，」他说。这个人比你想的老练得多。",
            { rep: 4, contact: { columnist: 14 }, fac: { press: 12 }, flags: ["archive_leak", "archive_hunter"] }),
          ok: aOut("材料有了第二个持有人。从这一刻起，你不能被单独处理了 —— 这就是你要的。",
            { rep: 2, contact: { columnist: 8 }, fac: { press: 8 }, flags: ["archive_leak"] }),
          ...
          fail: aOut("他把材料退了回来：「我现在不碰这个。」你多一个人知道了这件事，却没多一个帮手。",
            { rep: -2, contact: { columnist: -6 }, fac: { press: -4 }, flags: ["archive_leak"] }),
          critfail: aOut("他收下了。两周后你才知道他那张报纸的另一位股东，就是卷宗上第二个名字的姐夫。",
            { rep: -5, contact: { columnist: -12 }, fac: { press: -10 }, flags: ["archive_leak", "archive_seen"] })
        }
      },
      {
    ...
    choices: [
      {
        ...
        outcomes: {
          crit: aOut("他站起来，走到窗边，说了一个日期和一个人名。「那天你去见这个人，什么都不用带。」三周后你在一份任命名单上看到了自己的名字。",
            { tier: 1, rep: 10, fac: { establishment: 16, agency: 6 }, flags: ["archive_deal", "compromised"] }),
          ok: aOut("他打了一个电话，说了四句话。你得到了那个位置，也得到了一个你永远不会忘记的沉默。",
            { tier: 1, rep: 6, fac: { establishment: 12 }, flags: ["archive_deal", "compromised"] }),
          meh: aOut("他给了你一个次级的位置，还附带一句「先在那里做两年」。你听懂了这句话的另一半意思。",
            { rep: 3, fac: { establishment: 6 }, flags: ["archive_deal"] }),
          fail: aOut("他答应了，然后什么都没做。三个月后你才明白，他要的只是知道你会开什么价。",
            { rep: -4, fac: { establishment: -8 }, flags: ["archive_deal"] }),
          critfail: aOut("他把整段对话录了下来，而且没有剪掉你开口的那一句。这段录音从此躺在某个抽屉里，和你的卷宗放在一起。",
            { rep: -9, fac: { establishment: -14, press: -8 }, flags: ["archive_deal", "compromised", "scandal_3"] })
        }
      },
      {
        ...
        outcomes: {
          crit: aOut("你把全套材料交给了三家不同的机构，同一天。三个月后有一个委员会成立，一年后有一份报告。你的名字在报告的第一段，作为「材料来源」。你失去了很多朋友，得到了一个不会过期的东西。",
            { rep: 16, fac: { press: 20, base: 10, agency: -12, establishment: -14 }, flags: ["archive_expose", "whistleblower"] }),
          ok: aOut("它登出来了。你成了那个捅出事情的人 —— 有人从此不再跟你握手，也有人从此开始给你打电话。",
            { rep: 10, fac: { press: 14, base: 6, establishment: -10 }, flags: ["archive_expose", "whistleblower"] }),
          meh: aOut("它登了出来，但被做成了一桩「土地纠纷」。三个月后没有人记得。你付出了代价，什么都没换来。",
            { rep: 3, fac: { press: 6, establishment: -8, base: -4 }, flags: ["archive_expose"] }),
          fail: aOut("公开之前有人替你「校订」了材料，三个名字变成了一个。你成了被引导的那只手。",
            { rep: -3, fac: { press: -8, establishment: -6 }, flags: ["archive_expose"] }),
          critfail: aOut("材料是真的，但你的动机被写成了另一回事：为了那个位子。整件事最后变成了关于你的一场审查。",
            { rep: -8, fac: { press: -14, base: -10, establishment: -12 }, flags: ["archive_expose", "scandal_3", "investigation_open"] })
        }
      },
      {
        ...
        outcomes: {
          crit: aOut("你什么都没要，只说「我今天来是想认识您」。他送你到电梯口，握了很久的手。这张牌没打出去，但它从此每天都在替你说话。",
            { lev: 2, rep: 6, fac: { establishment: 10 }, flags: ["archive_hold"] }),
          ok: aOut("你把它扣住了。他会一直记得你手上有什么 —— 这比一次性的交换有用得多。",
            { lev: 1, rep: 3, fac: { establishment: 6 }, flags: ["archive_hold"] }),
          meh: aOut("你扣住了它。他从此不再为难你，也不再帮你。你换到的是「谁也不欠谁」。",
            { lev: 1, rep: 1, flags: ["archive_hold"] }),
          fail: aOut("你说「我什么都不要」的时候笑了一下。他看懂了那个笑。从那天起他开始准备第二份材料。",
            { lev: 1, rep: -4, fac: { establishment: -10 }, flags: ["archive_hold"] }),
          critfail: aOut("你把牌扣在手里，还四处暗示你手上有牌。一张没有打出去却被所有人看见的牌，等于没有牌 —— 只剩风险。",
            { rep: -6, fac: { establishment: -12, press: -8 }, flags: ["archive_hold", "scandal_2"] })
        }
      },
      {
        ...
        outcomes: {
          crit: aOut("你就着他的烟灰缸把复印件点着了，然后把灰搅开。他看了很久，说：「你是我这十年里见过的第二个这么做的人。」他后来真的成了你的长辈。",
            { rep: 10, lev: -1, fac: { establishment: 14, base: 8 }, flags: ["archive_burn"] }),
          ok: aOut("烧了。他点了点头，没有说谢谢。你们两个人那天都轻松了一点。",
            { rep: 5, lev: -1, fac: { establishment: 8 }, flags: ["archive_burn"] }),
          meh: aOut("你烧了复印件。原件还在档案馆里 —— 你们都清楚这一点，他也没说破。",
            { rep: 3, lev: -1, fac: { establishment: 4 }, flags: ["archive_burn"] }),
          fail: aOut("你烧得太快了，反而显得像在演。他送你出门的时候，眼神里有一点同情。",
            { rep: -1, fac: { establishment: -2 }, flags: ["archive_burn"] }),
          critfail: aOut("你烧的是复印件。原件被另一个知道你翻过档案的人取走了，而你在这一晚亲手放弃了自己唯一的筹码。",
            { rep: -6, lev: -1, fac: { establishment: -8 }, flags: ["archive_burn", "archive_seen"] })
        }
      }
    ]
  },
    ...
    choices: [
      {
        ...
        outcomes: {
          crit: aOut("你抢在所有人之前，在一次公开的场合把那段往事讲成了「两个人各退一步的故事」。没有人能反驳，因为版本是你先给的。",
            { rep: 8, fac: { establishment: 10, press: 8 }, flags: ["archive_owned"] }),
          ok: aOut("你找了个合适的机会把它摆平：一句轻描淡写的解释，一顿有见证人的饭。它从此变成了一件旧事。",
            { rep: 4, fac: { establishment: 6 }, flags: ["archive_owned"] }),
          meh: aOut("你没有主动处理。它就这样悬在那里，偶尔被人在背后提一句，永远没有一个说法。",
            { rep: -1, fac: { establishment: 2 } }),
          fail: aOut("你解释得太用力了。本来没人问的事，被你自己翻了出来。",
            { rep: -4, fac: { press: -8, establishment: -6 }, flags: ["scandal_1"] }),
          critfail: aOut("你当年拿到那个位置的方式，被人在一份小报的地方版上拼了出来。细节有错，但在政治上，有错不重要。",
            { rep: -8, fac: { press: -12, establishment: -12, base: -6 }, flags: ["scandal_3", "investigation_open"] })
        }
      },
      {
        ...
        outcomes: {
          crit: aOut("你在听证会上讲了三个小时，一个字都没有夸张。那天下午之后，你在两个阵营里同时失去了位置，也在整整一代人那里得到了一个名字。",
            { rep: 14, fac: { press: 16, civil: 12, base: 10, establishment: -16 }, flags: ["whistleblower", "archive_owned"] }),
          ok: aOut("你出庭了，说得比材料本身少。这样也好 —— 少的那部分，别人会替你说。",
            { rep: 8, fac: { press: 10, base: 5, establishment: -10 } }),
          meh: aOut("你出庭了，然后那件事被并进了另一桩更大的案子。你的部分只占了两页。",
            { rep: 3, fac: { press: 5, establishment: -6 }, hp: -4 }),
          fail: aOut("有人在你之前出庭，讲了一个和你完全不同的版本。你成了那个「说法比较多」的证人。",
            { rep: -4, fac: { press: -8, base: -6 } }),
          critfail: aOut("交叉询问的时候，对方只用了二十分钟，就把你的动机拆成了「没拿到那个位置的人」。这句话后来跟着你很久。",
            { rep: -9, hp: -6, fac: { press: -12, base: -10, establishment: -10 }, flags: ["scandal_3"] })
        }
      },
      {
      ...
      },
      {
        ...
        outcomes: {
          crit: aOut("七年之后，在一个完全无关的场合，有人提起了那件事，用的说法是「那年那件谁也没搞清楚的事」。没人接话。它真的老了。",
            { rep: 5, fac: { base: 5, establishment: 4 } }),
          ...
          fail: aOut("你不再提它，但你开始回避某些场合、某些人、某几条街。你的日程表慢慢变得比你想的小。",
            { rep: -2, hp: -3, fac: { base: -3 } }),
          critfail: aOut("你以为它老了，但一直有人在替它续命 —— 每隔几年就有人把那份材料复印一遍。它不会老，它只是在等。",
            { rep: -5, hp: -4, fac: { agency: -6 }, flags: ["scandal_2"] })
        }
      }
    ]
  }
