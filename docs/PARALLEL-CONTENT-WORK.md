...
1980—1990 共 **21 个定点**（`21-worldline.js` 4 个 + `110-line-1980s.js` 17 个）
= **≈1.9 个/年**，其中 `major` ≈ 1/3。
⇒ 1991—2024 共 34 年 ⇒ **60—68 个定点 / 约 65 张新卡**，`major` 占 1/3。
...
| B4 | 2003—2006 | `events/123-line-2003-06.js` | 8 | 伊拉克开战、无 WMD 结论（杜芬报告）、加州罢免、东北大停电、阿布格莱布、萨达姆被捕、2004 大选、卡特里娜（**该带最高优先级**）、2005 后布什支持崩、2006 中期选举民主党翻盘 |
| B5 | 2007—2010 | `events/124-line-2007-10.js` | 8 | 弗吉尼亚理工、次贷裂缝、雷曼与问题资产救助法（存量 `2008_*` 大量卡：**只 pin 不重写**）、2008 大选、刺激方案与 AIG 奖金、H1N1、胡德堡、维基解密、墨西哥湾漏油、SB1070、2010 茶党浪潮（存量 `2008_tea_party` / `wave_tea_rally` 可 pin） |
| B6 | 2011—2014 | `events/125-line-2011-14.js` | 8 | 图森枪击案、本·拉丹被击毙、债务上限与评级下调、占领华尔街（存量 `wave_occupy_low` pin）、桑迪·胡克、班加西、飓风桑迪、2012 大选、波士顿爆炸、斯诺登、政府停摆、弗格森 |
| B7 | 2015—2018 | `events/126-line-2015-18.js` | 8 | 同婚合法化、查尔斯顿教堂枪击与降旗、圣贝纳迪诺、2016 大选（存量 `soc16_election` **只 pin**）、奥兰多、弗林特水危机、科米被解职、夏洛茨维尔、哈维/玛丽亚、拉斯维加斯枪击、帕克兰与拥枪游行、2018 中期选举 |
| B8 | 2019—2021 | `events/127-line-2019-22.js` | 8 | 第一次弹劾、埃尔帕索/代顿、新冠疫情（**该带最高优先级**）、弗洛伊德与夏天、邮寄票（存量 `soc20_mailin` pin）、2020 大选、国会山、阿富汗撤军、供应链与通胀起 |
| B9 | 2022—2024 | `events/128-line-2022-24.js` | 7 | 多布斯推翻罗伊、CPI 峰值、罗伯小学枪击、2022 中期选举、硅谷银行与存款搬家、债务上限边缘、麦卡锡被罢免、毛伊野火、10·7 与校园对立、特朗普遇刺未遂、换候选人、飓风海伦妮/米尔顿、2024 大选 |
...
`choice-audit` 的基线（合并方核对用）：**多选项事件 196 ｜ 占优 0 ｜ 过平 0 ｜ 同轴 3**。
...
- **同时最多 6 个 worker**（Track B 4 个 + Track A 2 个）。
  理由：20 逻辑核里有 8 个性能核，门禁是单线程 JS；超过 6 个并发后
  每个 worker 的校验轮次会被拖长，总吞吐反而下降；且合并方是**串行**瓶颈。
- 合并方每合并 2—3 个分支跑一次 `--games=20` 全量 + `--lang=en --games=1`。
- 若要提速：Wave 制（§4 的 9 个带分 2 波，每波 5/4 个），合并方按 §3 串行入库。
