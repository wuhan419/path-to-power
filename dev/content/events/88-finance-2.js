...
POTUS.define("event", [
    ...
    choices: [
      {
        ...
        outcomes: {
          crit: out("钱到账的当天，你顺手把交割文书从头到尾读了一遍，还抄了一份留底。半年后有人来打听时，你是全家族唯一说得清的人。",
            { fun: 220000, rep: 3, fac: { base: 4 }, flags: ["windfall_kept"] }),
          ok: out("钱进了账，窟窿堵上了。你把那页「不要询问」的附言折起来收进了抽屉最底层。",
            { fun: 160000, flags: ["windfall_kept"] }),
          meh: out("钱到得比说好的少了两成——「税费和杂费」。你没处问，也懒得问。",
            { fun: 90000, flags: ["windfall_kept"] }),
          fail: out("交割拖了四个月，你的下半年是在催款电话里过的。钱最后还是来了，只是来晚了。",
            { fun: 60000, hp: -3, flags: ["windfall_kept"] }),
          critfail: out("钱到账第三周，一家旧商行的清算人找上门：这笔遗产里有一笔「有待确认的债权」。你没发大财，倒是先请了律师。",
            { fun: 20000, rep: -4, hp: -4, fac: { commercial: -6 }, flags: ["windfall_kept", "investigation_open"] })
        }
      },
      {
        ...
        outcomes: {
          crit: out("你花了三周，把那笔资产三十年的履历拼了出来。它不算干净，但说得清——你只收下说得清的那一半，把剩下的捐给了姑婆生前常去的教学。报纸给了你一小段表扬。",
            { fun: 90000, rep: 6, fac: { press: 8, base: 6, church: 4 } }),
          ok: out("查清了：钱是老派的、慢的、无趣的——租金和利息攒了四十年。你收得心安理得。",
            { fun: 130000, rep: 3, fac: { base: 3 } }),
          meh: out("查了个半懂。你收下了钱，也在心里留了一个没关上的抽屉。",
            { fun: 100000, flags: ["windfall_kept"] }),
          fail: out("越查越乱。执行人最后不耐烦了：「要么签字，要么放弃。」你在压力下签了。",
            { fun: 70000, hp: -2, flags: ["windfall_kept"] }),
          critfail: out("你查到的那半截，恰好是别人不想让任何人碰的旧账。信封还没拆完，就有人托话来请你「适可而止」。",
            { fun: 30000, rep: -3, fac: { criminal: -8 }, contact: { fixer: 6 }, flags: ["compromised"] })
        }
      },
      {
   ...
   * ======================================================================== */
  {
    ...
    unique: true, grade: "major", category: "finance",
    ...
    choices: [
      {
        ...
        outcomes: {
          crit: out("你付了钱，还让他签了一份「两清」的确认函——你不知从哪来的冷静。他收了函，真的再没出现过。",
            { rep: 2, fac: { criminal: 4 }, notFlags: ["windfall_kept"] }),
          ok: out("钱付了，人走了。你在停车场坐了半个小时才发动车。",
            { notFlags: ["windfall_kept"], flags: ["compromised"] }),
          meh: out("他嫌少。你又加了一笔，事情才算完。",
            { fun: -80000, notFlags: ["windfall_kept"], flags: ["compromised"] }),
          fail: out("你付了钱，三个月后换了一个「新的资产管理人」来接手「同一份安排」。",
            { fun: -120000, rep: -3, fac: { criminal: -6 }, flags: ["compromised"] }),
          critfail: out("付款的记录成了新的把柄。从此你不是被一份旧遗嘱拿住，而是被你自己的一笔支出拿住。",
            { fun: -120000, rep: -8, fac: { criminal: 8, press: -6 }, flags: ["compromised", "black_money"] })
        }
      },
      {
        ...
        outcomes: {
          crit: out("你先开了记者会，再把材料交上去。故事的主角从「收黑钱的政客」变成了「敲诈候选人的骗子」。那位管理人当月被带走协助调查。",
            { rep: 10, voters: { diehard: 600, warm: 1200 }, fac: { press: 14, base: 8, criminal: -12 }, notFlags: ["windfall_kept"], flags: ["whistleblower"] }),
          ok: out("调查立案了，你也挨了几天的骂。但你把「被拿住」变成了「主动交出」，那扇门从此关上了。",
            { rep: 4, voters: { diehard: 300 }, fac: { press: 8, criminal: -8 }, notFlags: ["windfall_kept"] }),
          meh: out("委员会收了材料，然后没有下文。你暴露了自己，却没换来一个说法。",
            { rep: -2, fac: { press: 4 }, notFlags: ["windfall_kept"] }),
          fail: out("记者先写的是你「收过一笔说不清的遗产」，敲诈的事放在了第七段。你疼了整整一个季度。",
            { rep: -7, voters: { warm: -1500 }, fac: { press: -6, base: -6 }, flags: ["scandal_2"], notFlags: ["windfall_kept"] }),
          critfail: out("材料交上去第二天就泄了，泄出来的版本只剩你收钱那一半。管理人没事，你成了那条街的笑话。",
            { rep: -12, voters: { warm: -2500, oppose: 1200 }, fac: { press: -10, base: -10, criminal: 6 }, flags: ["scandal_3", "investigation_open"], notFlags: ["windfall_kept"] })
        }
      },
      {
        ...
        outcomes: {
          crit: out("你只回了一句话：「请你的律师和我的律师谈。」两个月后那份批注被鉴定为后添的——他赌输了，而且输了整条街的生意。",
            { rep: 6, fac: { criminal: -14, base: 6 }, notFlags: ["windfall_kept"] }),
          ok: out("你拖住了。他等的成本比你要的高，半年后他自己消失了。",
            { rep: 2, notFlags: ["windfall_kept"] }),
          meh: out("僵持着。没有交易，也没有了结，这件事变成了你日程表上一个删不掉的备注。",
            { hp: -3, flags: ["compromised"] }),
          fail: out("他把批注的复印件寄给了三家报社，附上你的名字。没有一家核实，但三家都「听说了」。",
            { rep: -6, fac: { press: -8 }, flags: ["scandal_2"] }),
          critfail: out("他掀了桌：交割文书、批注、还有你当年「没有提问」的签收记录，一次性全放了出来。",
            { rep: -13, voters: { warm: -2000, oppose: 1000 }, fac: { press: -12, base: -8, criminal: 8 }, flags: ["scandal_3", "investigation_open"], notFlags: ["windfall_kept"] })
        }
      }
    ]
  },
   ...
   * ======================================================================== */
  {
    ...
    grade: "major", category: "finance", medium: ["print", "tv"],
    title: "对不上的一栏",
      ...
      },
      {
        ...
        outcomes: {
          crit: out("律师团把每一笔现金支出都配上了说法和凭证。结案函写得像表扬信。你花的钱买回了完整的清白。",
            { rep: 7, fac: { commercial: 10, establishment: 8, base: 4 }, flags: ["audit_clean"] }),
          ok: out("拖了八个月，以补税少量、不加罚结案。钱花得肉疼，事算是过去了。",
            { fun: -120000, rep: 2, fac: { commercial: 5 }, flags: ["audit_clean"] }),
          meh: out("程序赢了一半：补税照补，罚金减了。你赢的是折扣，不是说法。",
            { fun: -250000, rep: -1 }),
          fail: out("律师的每一封函都被记者拿到。八个月里你上过六次版面，没有一次是你写的稿。",
            { rep: -6, fac: { press: -10, commercial: 4 } }),
          critfail: out("律师团队里有人把你的底稿卖了个好价钱。你花了钱，买回来一份起诉书。",
            { rep: -11, voters: { warm: -1800, oppose: 800 }, fac: { press: -12, commercial: -8 }, flags: ["scandal_4", "investigation_open"] })
        }
      },
      {
        ...
        outcomes: {
          crit: out("你解雇了他，公开说是「记账失职」，私下替他付了转行的路费。他签了保密协议，至今没开口。",
            { rep: 1, contact: { fixer: 4 }, flags: ["audit_clean"] }),
          ok: out("会计背下了「工作失误」，审计以整改结案。你在办公室多坐了十分钟才去开下一个会。",
            { rep: -2, flags: ["audit_clean", "compromised"] }),
          meh: out("他认了失误，但认得心不甘情不愿。你看得出来，这个故事会有下半部。",
            { rep: -4, flags: ["compromised"] }),
          fail: out("他不背。他把底稿的「存在」透露给了记者——不是内容，是存在。这下人人都知道有一本真账。",
            { rep: -7, fac: { press: -8 }, flags: ["scandal_2", "leaker_suspect"] }),
          critfail: out("他反手成了污点证人。你推出去的那个人，回来的时候带着案卷和豁免协议。",
            { rep: -13, voters: { warm: -2200, oppose: 1500 }, fac: { press: -12, agency: -6, base: -10 }, flags: ["scandal_4", "investigation_open", "whistleblower"] })
        }
      },
      {
        ...
        outcomes: {
          ...
          fail: out("期限过了。不回应本身成了记录在案的事实，罚金和传闻一起涨。",
            { fun: -100000, rep: -5, flags: ["scandal_2"] }),
          critfail: out("你的沉默给了对手整整一个季度。等传票下来的时候，公众早就判完了。",
            { rep: -9, voters: { warm: -1500, oppose: 800 }, fac: { press: -10, base: -8 }, flags: ["scandal_3", "investigation_open"] })
        }
      }
    ]
  },
    ...
    choices: [
      {
        ...
        req: { fun: 1000000 }, stake: { fun: true, ap: true },
        outcomes: {
          crit: out("立项公告那天，你的地块估值为买入价的三倍半。你提前两个月就办好了脱手文书——赚完就走，一步没多留。",
            { funMul: 2.5, rep: 3, fac: { commercial: 12 }, flags: ["land_deal"] }),
          ...
          fail: out("第二套方案胜出——仓库区真的还是仓库区。你割肉离场，中间人的电话再没打通过。",
            { funMul: -0.7, rep: -3, fac: { commercial: -6 } }),
          critfail: out("你买入的时机太准了，准到有记者去查了成交日和草案签发日。两个日期之间只差四天。故事不需要再多的素材了。",
            { funMul: -0.9, rep: -10, voters: { warm: -1500, oppose: 800 }, fac: { press: -12, base: -10, commercial: 4 }, flags: ["scandal_3", "investigation_open", "land_deal"] })
        }
      },
      {
        ...
        req: { fun: 300000 }, stake: { fun: true },
        outcomes: {
          crit: out("仓位小，出手快，账面干净。立项那天你小赚一笔，还顺手认识了对岸的两个业主——他们后来帮过你大忙。",
            { funMul: 1.6, rep: 2, fac: { commercial: 6 }, contact: { fixer: 5 } }),
          ...
          critfail: out("即便只是一间仓库，成交通知也进了本地报纸的地产版。标题里你的职务比金额显眼。",
            { funMul: -0.6, rep: -5, fac: { press: -8, base: -4 }, flags: ["scandal_2"] })
        }
      },
      {
   ...
   * ======================================================================== */
  {
    ...
    grade: "major", category: "finance", unique: true, medium: ["print", "tv", "internet"],
    title: "自己的盘子",
    ...
    choices: [
      {
        ...
        cost: { fun: 800000, ap: 2 }, stake: { fun: true, fav: true },
        outcomes: {
          crit: out("六个月后，你的委员会成了本党最锋利的一把刀——它资助的七个候选人赢了五个。党内再也没人敢把你当配角。",
            { rep: 12, lev: 1, voters: { diehard: 1200, warm: 800 }, fac: { establishment: 14, commercial: 12, base: 8 }, contact: { lobbyist: 16 }, flags: ["pac_leader", "kingmaker"] }),
          ok: out("盘子立起来了，头两场助选赢了一场半——赢的那场赢得漂亮，输的那场输得体面。找上门的钱开始变多。",
            { rep: 6, voters: { diehard: 500 }, fac: { establishment: 8, commercial: 8 }, contact: { lobbyist: 10 }, flags: ["pac_leader"] }),
          meh: out("委员会开了张，募款刚够养团队。它暂时还只是一块招牌，不是一把刀。",
            { rep: 2, fac: { commercial: 4 }, contact: { lobbyist: 5 }, flags: ["pac_leader"] }),
          fail: out("首笔捐款的来源被记者顺藤摸到了一个你不想沾边的名字。盘子还没热就得先灭火。",
            { rep: -6, fac: { press: -8, commercial: -6 }, flags: ["pac_leader", "scandal_2"] }),
          critfail: out("「不协调」那条线被一封邮件戳穿了——你的竞选经理和委员会总监每周三同一时间通话。委员会被吊销，你的名字进了案卷标题。",
            { rep: -12, voters: { warm: -2000, oppose: 1200 }, fac: { press: -12, establishment: -12, agency: -6 }, flags: ["scandal_4", "investigation_open"], notFlags: ["pac_leader"] })
        }
      },
      {
        ...
        outcomes: {
          crit: out("小盘子运转得意外地好：钱干净、账清楚、每次助选都挑得准。三年后回头看，这个不起眼的开局比任何大干一场都稳。",
            { rep: 7, voters: { diehard: 600 }, fac: { establishment: 8, base: 5 }, contact: { lobbyist: 10 }, flags: ["pac_leader"] }),
          ok: out("招牌挂上了，通道通了。第一年它替三个地方选举送了钱，也替你攒了三个人情。",
            { rep: 4, fac: { establishment: 5, commercial: 4 }, contact: { lobbyist: 7 }, flags: ["pac_leader"] }),
          meh: out("募款平平。委员会成了名片上的一行字，聊胜于无。",
            { rep: 1, contact: { lobbyist: 3 }, flags: ["pac_leader"] }),
          fail: out("低调没换来安全——第一份捐款申报就填错了一栏，罚金加整改，招牌蒙了灰。",
            { fun: -100000, rep: -3, fac: { establishment: -4 } }),
          critfail: out("说客里有一个同时替三方管钱，你的「干净通道」被卷进了他的对账风波。你提前切割才全身而退，但招牌没保住。",
            { rep: -7, fac: { press: -8, commercial: -6, establishment: -6 }, flags: ["scandal_2"] })
        }
      },
      {
        ...
        cost: { fun: 300000 }, stake: { fav: true },
        outcomes: {
          crit: out("钱像水一样进来。你定好了规矩：只见、不承诺、不落笔。半年里你见的人比党内大佬还多，而每一步都擦着线却没越线。",
            { fun: 1500000, rep: 2, fac: { commercial: 16, establishment: 6, criminal: 4 }, contact: { lobbyist: 14 }, flags: ["pac_leader", "bought"] }),
          ok: out("门票生意做起来了。你见了一屋子想要东西的人，也记住了每一张脸想要什么——这些以后都是牌。",
            { fun: 800000, fac: { commercial: 10 }, contact: { lobbyist: 10 }, flags: ["pac_leader", "bought"] }),
          meh: out("赚了钱，见了人，但「见面的价码」这个说法开始在圈子里流传。",
            { fun: 400000, rep: -2, fac: { commercial: 6, press: -4 }, flags: ["pac_leader", "bought"] }),
          fail: out("一位捐了大钱的先生在见面后三天拿到了一份合同。时间线太过好看，记者把它画成了一张图表。",
            { rep: -7, fac: { press: -10, commercial: 4 }, flags: ["scandal_2", "pac_leader"] }),
          critfail: out("门票的价目表被人录了音。那段录音在电视上放了一周，在网络上传了一年。你成了「明码标价」这个词的配图。",
            { rep: -14, voters: { warm: -3000, oppose: 2000 }, fac: { press: -14, base: -12, commercial: 6 }, flags: ["scandal_4", "tape_out", "investigation_open"] })
        }
      },
      {
      ...
      },
      {
        ...
        outcomes: {
          crit: out("钱当天到账，连一句多余的话都没有。你三个月内连本带利还清——他握着你的手说：「我就喜欢还不拖的人。」这一笔他没打算赚别的。",
            { fun: 250000, contact: { shark: 10 } }),
          ok: out("过桥的钱到了，桥过了。利息高得吓人，但你活着走到了对岸。",
            { fun: 150000, contact: { shark: 8 }, flags: ["owes_shark"] }),
          meh: out("他给了钱，但要了你未来助选名单上「三个职位」的第一拒绝权。你签了。",
            { fun: 100000, contact: { shark: 5 }, flags: ["owes_shark", "compromised"] }),
          fail: out("利息、罚则、连带担保——条款一条比一条狠。你借到了钱，也把缰绳递了出去。",
            { fun: 80000, rep: -3, contact: { shark: 4 }, flags: ["owes_shark", "compromised"] }),
          critfail: out("他还真来了——带着两个律师和一份「逾期则接管债务」的合同。账上见底第五天，你的竞选多了一个真正的老板。",
            { fun: 60000, rep: -6, contact: { shark: 6 }, flags: ["owes_shark", "compromised", "black_money"] })
        }
      },
      {
        ...
        outcomes: {
          crit: out("你亲自裁，一个一个谈，遣散费给足、推荐信写好。留下的十二个人反而空前地紧——后来的很多年里，他们管那两周叫「我们的冬天」。",
            { fun: 40000, rep: 5, fac: { base: 6, labor: 4 }, flags: ["lean_team"] }),
          ok: out("一半的人走了，账平了。竞选变慢了，但没死。",
            { fun: 20000, rep: 2, fac: { labor: -3 }, flags: ["lean_team"] }),
          meh: out("裁得手忙脚乱，走了两个不该走的人。账是平了，阵脚也乱了。",
            { rep: -2, fac: { labor: -5 }, flags: ["lean_team"] }),
          fail: out("被裁的人里有会讲故事的那个。他的版本上了三家媒体：节省的成本，全变成了名声的窟窿。",
            { rep: -6, fac: { press: -8, labor: -8 }, flags: ["scandal_2"] }),
          critfail: out("裁员的名单被指「专裁忠于你的人、留溜须拍马的人」。你是在省成本，全城听到的却是清洗。核心团队保住了，你的信誉没保住。",
            { rep: -10, voters: { warm: -1500 }, fac: { base: -10, labor: -12 }, flags: ["scandal_2"] })
        }
      },
      {
        ...
        outcomes: {
          crit: out("你站在借来的场地里讲了四十分钟，讲钱、讲窘境、讲为什么还要做下去。散场时捐款桶装不下了——第二天本地报纸的标题是《他自己把话说破了》。",
            { fun: 300000, rep: 10, voters: { diehard: 800, warm: 1000 }, fac: { base: 10, press: 8 } }),
          ok: out("场子坐了七成，钱够撑两个半月。你把这一天讲给了以后遇见的每一个困难时刻。",
            { fun: 150000, rep: 5, voters: { diehard: 300 }, fac: { base: 6 } }),
          meh: out("来的人不多，捐的钱刚够场地费。你至少确认了谁真的会来。",
            { fun: 30000, voters: { diehard: 100 } }),
          fail: out("场子空了一半，对手的人还来拍了照。照片配的标题是《他的时代结束了》。",
            { rep: -6, voters: { warm: -800 }, fac: { press: -8, base: -5 }, flags: ["fallen"] }),
          critfail: out("你在台上讲到嗓子劈了，台下稀稀拉拉。第二天的报道只有一句：那位先生的募款会，到场的记者比支持者多。",
            { rep: -11, hp: -5, voters: { warm: -1800, oppose: 600 }, fac: { press: -10, base: -10 }, flags: ["fallen"] })
        }
      }
    ]
  }
