...
/* 学生贷款是贯穿整个生涯的东西——不限某个年代。默认/收账一幕应在玩家深度逾期的
   任意年代都能被抽到，故走全时代（与 91-scandal-2 / 96-crisis-2 同惯例）。 */
const FIN2_ALL = Object.keys(POTUS.reg.era);
...
POTUS.define("event", [
   ...
   * ======================================================================== */
  {
    ...
    body: "律师事务所的信封很厚，里面只有一页纸：一位你几乎忘了的远房姑婆留给你一笔遗产，\n" +
      "够你把眼下的窟窿全堵上，还剩一点。全家族只有你在从政——你的名字，是能被人拿来用的。\n" +
      "执行人附了一句：「委托人希望您不要询问这笔资产的构成」——钱在变现前是股票、房产还是旧账。\n" +
      "「不要询问」是遗愿的条件，还是某个活人的习惯？",
    choices: [
      {
   ...
   * ======================================================================== */
  {
    ...
    body: "一个自称「资产管理人」的人约你在一家不吵的餐厅，把复印件推过桌面：\n" +
      "你当年那笔遗产交割文书的最后一页，多了一行你从没见过的批注——所谓附加安排，遗嘱外的私人备忘，法律效力模糊。\n" +
      "「委托人当年有一个安排，现在到期了。」他要一笔管理费，还要你在委员会里保持现状。\n" +
      "文书是真的，批注也可能是真的——他赌的就是你不敢去验。这页纸若交给该交的地方，会先炸到谁？",
    choices: [
      {
   ...
   * ======================================================================== */
  {
    ...
    body: "通知函只有两段：税务部门「依法」调阅你近三年的申报材料，\n" +
      "理由栏写着——「申报生活水准与报告收入存在显著差异」。这种生活水准审计看日子不看账：车、房、旅行比账本诚实。\n" +
      "你的会计在电话里沉默了很久：三年里有十一笔支出走了现金，其中三笔你说不清用途。\n" +
      "审计不等于指控——结案可以是补税，也可以是案卷转检方。那三笔现金会先惊动税务局还是记者，你说了不算。",
    choices: [
      {
   ...
   * ======================================================================== */
  {
    ...
    body: "介绍你来的中间人说得很直白：河对岸规划中的大桥一旦立项，那片仓库区就是下一个城区。\n" +
      "地价现在只有立项后估值的三成——开发商要的就是你这种「有分量」的早期买家撑盘。\n" +
      "规划图还是草案，草案上却有你委员会里见过面的签名。「买不买，这一周就要定。」\n" +
      "有人说大桥还有第二套走向，仓库区在那套方案里仍是滩涂。政治人物炒地赚的是信息差——信息差也正是别人盯你的地方。",
    choices: [
      {
   ...
   * ======================================================================== */
  {
    ...
    body: "两个说客把方案摊在你面前：成立一个领导型政治行动委员会——以你的名义募款、独立于你的竞选。\n" +
      "「捐款上限管得住给候选人的钱，管不住给委员会的，」其中一人笑着说，\n" +
      "「但委员会的钱也不能直接进您的竞选账上——这条线我们替您布好。」\n" +
      "登记、章程、合规官全是现成的，连首笔捐款人都已物色好。立不起委员会的人没有分量，你有了。\n" +
      "「从今天起，您可以决定别人的选举。」只是盘子做大之后，是你用它，还是它用你？",
    choices: [
      {
   ...
   * ======================================================================== */
  {
    ...
    body: "财务主管把最后一页报表推过来，上面只有一个加框的数字，比你觉得还能撑的数少了两位。\n" +
      "「三笔说好的捐款同一周全黄了，债主还不知道这个数字。」她一条一条念，\n" +
      "「房租下周到期，印刷厂要现结，还有两笔工资。先生，我们不是经营不善，我们是没钱了。\n" +
      "下周五之前，您得给团队一个数字。」拖欠一旦见报，捐款就更进不来。\n" +
      "城里还有个专等这种时候出手的放贷人，他要的从来不只是利息。而你办公室里有几个人，这周已经悄悄更新过简历。",
    choices: [
      {
  ...
  /* ==========================================================================
   * 6) 学贷长期违约 —— cond 门控：欠款还在 + 连续逾期满 lateMonths 才找上门
   *    还原「奥巴马当总统还在还学贷」的另一面：拖久了，催收和征信会变成一个
   *    公开话题。这一幕不直接致局，只把「艰难度日」的代价摆到台面上。
   * ======================================================================== */
  {
    id: "fin2_loan_default", era: FIN2_ALL, tierMin: 0, tierMax: 6, weight: 10,
    grade: "mid", valence: "bane", dyn: true, unique: true, category: "finance",
    cond: function (G) {
      const s = (POTUS.balance() || {}).studentLoan || {};
      return (G.debt || 0) > 0 && (G.loanLate || 0) >= (s.lateMonths || 12);
    },
    title: "一笔多年没还清的学生贷款，被人翻了出来",
    body: "一家本地媒体打来电话，语气很客气，问题很不客气：一位「公开承诺为普通人发声」的候选人，\n" +
      "名下有一笔欠了多年、如今已进入逾期催收的学生贷款。「您是忘了，还是根本不在乎？」\n" +
      "记者手里是公开记录，这笔账瞒不住——你能选的只是怎么讲。还上一大笔能当天止住催收，但那笔钱得从别处挖一个窟窿。\n" +
      "你盯着屏幕上那串数字：利息滚得比你任何一次演讲都勤。对手已把它贴上议题清单，等你先哭还是先硬。",
    choices: [
      {
        id: "own_it", text: "开诚布公：把这笔旧账摊到阳光下讲",
        note: "免费保底，赌人心。承认它，把它讲成和你选民一样的处境。",
        base: 0.5, mods: [{ src: "attr", key: "CHA", w: 0.5 }, { src: "attr", key: "INTG", w: 0.3 }],
        outcomes: {
          crit: out("你约来那家媒体，把贷款单、利率、这些年的还款记录一张张摊开：「我花了这么多年还没还完它，这就是你们要选的人真实的日子。」第二天，转发最多的一句话是「他至少没装」。",
            { rep: 1.25, fac: { base: 10, press: 6 }, attr: { INTG: 4 }, flags: ["own_debt"] }),
          ok: out("你没有躲，故事就从「丑闻」降格成了「插曲」。有人买账，有人没买，但没人能说你不坦诚。",
            { rep: 0.5, fac: { base: 4 } }),
          meh: out("你讲了，可语气里那点委屈还是被听出来了。人们不讨厌欠钱的人，讨厌的是把自己摆成受害者的候选人。",
            { rep: -0.1 }),
      ...
      {
        id: "settle", text: "想办法筹一笔钱，一次性把贷款结清",
        note: "砸钱抹平。能立刻止住催收，但那笔钱要从别的地方挖走。",
        base: 0.62, mods: [{ src: "attr", key: "CUN", w: 0.45 }, { src: "attr", key: "INT", w: 0.25 }],
        cost: { fun: 0.8 },
        outcomes: {
          crit: out("你东拼西凑，一次性把本息结清，催收当天停了。你甚至有余裕把还款记录大大方方贴出来：「处理完了，下一个问题。」",
            { rep: 0.5, fac: { establishment: 4 }, debt: -999999 }),
          ok: out("钱到位了，贷款清零，催收闭嘴。只是账上那个新挖的窟窿，接下来几个月你得勒着腰带填。",
            { debt: -999999, rep: 0.1 }),
          meh: out("凑钱的过程有点狼狈，几个朋友被开口借了个遍。最后是还上了，可「他为了还贷款到处借钱」这话，也一并传了出去。",
            { debt: -999999, rep: -0.4, fac: { base: -4 } }),
      ...
      {
        id: "stall", text: "避而不谈，让律师去和对方周旋",
        note: "免费。眼下不出丑，代价是利滚利，和一颗随时会再爆的雷。",
        base: 0.55, mods: [{ src: "attr", key: "CUN", w: 0.35 }],
        outcomes: {
          crit: out("律师把程序拖得很漂亮，这一波热度过去前，报道没了下文。你赌对了——暂时。",
            { rep: 0.2 }),
          ok: out("你没回应，故事凉得比预想快。只是那笔贷款还在暗处继续滚利息。",
            { rep: 0.05 }),
          meh: out("沉默被读成了默认。对手不费一枪一弹，把「他拒绝回应自己的债务」这句话，留给了选民自己想象。",
            { rep: -0.4, fac: { press: -4 } }),
