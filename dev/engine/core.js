...
POTUS.quickSave = function () {
  try {
    ...
    if (name) {
      POTUS.G.saveName = name;                     // 名称进存档体：读档列表按它显示人话名
      POTUS.G.saveAt = Date.now();
      localStorage.setItem(SAVE_KEY + "_" + btoa(unescape(encodeURIComponent(name))).slice(0, 20), POTUS.serialize());
      alert("已保存：" + name);
    }
  } catch (e) { alert("保存失败：" + e.message); }
};
/* 存档的简要状态（读档列表用）：名字 · 时代 年月 · 职名（层级）——解析失败退回 key */
POTUS.saveBrief = function (raw) {
  try {
    const G = JSON.parse(raw);
    const era = (POTUS.reg.era[G.era] || {}).name || G.era || "";
    const office = (POTUS.officeName && POTUS.G) ? null : null;   /* officeName 依赖运行态，列表里用静态表 */
    const oTable = POTUS.reg.office || {};
    const fb = POTUS.balance().officeFallback || [];
    const oHit = oTable[(G.track || "*") + "_" + (G.tier || 0)] || oTable["*_" + (G.tier || 0)];
    const oName = oHit ? (typeof oHit === "string" ? oHit : oHit.name) : (fb[G.tier || 0] || ("T" + (G.tier || 0)));
    return (G.saveName || "未命名") + "　·　" + era + " " + (G.year || "") + "年" + (G.month || 1) + "月" +
      "　·　" + oName + "（T" + (G.tier || 0) + "）　·　声望" + (G.rep || 0) + " 资金$" + ((G.fun || 0) / 1000).toFixed(0) + "k";
  } catch (e) { return null; }
};
...
POTUS.openLoad = function () {
  ...
  /* 自动存档 + 手动档都显示 MC 简要状态（名称在存档体里，不再显示编码后的 key） */
  const auto = localStorage.getItem(SAVE_KEY + "_auto");
  if (auto) {
    const brief = POTUS.saveBrief(auto);
    html += '<button class="btn saveline" onclick="POTUS.doLoad(\'auto\')"><b>自动存档</b><small>' +
      (brief || "（空的）") + "</small></button>";
  }
  keys.forEach(function (k) {
    const brief = POTUS.saveBrief(localStorage.getItem(k) || "");
    html += '<button class="btn saveline" onclick="POTUS.doLoad(\'' + k + '\')"><b>' +
      ((brief && brief.split("　·　")[0]) || k.replace(SAVE_KEY + "_", "")) + "</b><small>" +
      (brief ? brief.split("　·　").slice(1).join("　·　") : "（无法读取）") + "</small></button>";
  });
